...
#define PIFOURTH 0.785398163397

#define IF_FREQ 152000

#define BAND_WIDTH_HZ 12


// defaults (gp, proper settings for narrowband sstv)
#define DEFAULTFREQ 87.5
#define DEFAULTRATE 11025
#define DEFAULTBANDWIDTH 1
// sekio: I determined this by brute force of course
#define DEFAULTTIMING 1415.70f
...
void setup_fm(int state)
{
    ...
    union GPCTLUNI setupword;

    setupword.g = {6 , 1, 0, 0, 0, (char)state,(char)0x5a};

    ACCESS(CM_GP0CTL) = setupword.i;
}
...
public:
    ...
    static const int BUFSIZE = 1024;  // sekio: multiples of 2 !!!!!!!
...
// sekio : do you really expect me to use some gnu radio nonsense
...
// Takes 2 input signals and stereo modulates it.
...
public:
    ...
    StereoModulator(SampleSink* nnext): 
        next(nnext), bufferOwner(0), bufferLen(0), state(0) {
        for (int i=0; i<16; i++) {
            sinLut[i] = sin((double)i*PIFOURTH);
        }
    }
...
void playWav(char* filename, float samplerate, float bandwidth, float timing_correction, bool stereo)
{
    ...
    if (stereo) {
      StereoModulator* sm = new StereoModulator(new Outputter(IF_FREQ,bandwidth,timing_correction));
      ss = new StereoSplitter( 
        // left
        new PreEmp(samplerate, new Resamp(samplerate, IF_FREQ, sm->getChannel(0))), 
        
        // Right
        new PreEmp(samplerate, new Resamp(samplerate, IF_FREQ, sm->getChannel(1)))
      );
    } else {
...
{
    int opt;
    int stereoflag=0,nowarn=1;
    double frequency, samplerate, bandwidth, timing;
    frequency = DEFAULTFREQ;
    samplerate= DEFAULTRATE;
    bandwidth = DEFAULTBANDWIDTH;
    timing    = DEFAULTTIMING;

    while ((opt = getopt(argc, argv, "f:s:b:t:rm")) != -1) {
        switch (opt) {
        case 'f':
    ...
    if ((optind < argc) && (nowarn==1)) {
      printf("We get %s signal !!!\n\t - %.2f Mhz carrier (b/w approx %.0f khz)\n\t - %.0f hz input signal\n\t - %.2f fudgefactor\n", stereoflag?"stereo":"mono",frequency,bandwidth*BAND_WIDTH_HZ,samplerate,timing);
      setup_fm(1);
      setupDMA(frequency);
      playWav(argv[optind], samplerate,bandwidth,timing, stereoflag);

    } else {
      fprintf(stderr, "Usage:   %s wavfile.wav [-f freq] [-s sample rate] [-b band width] [-t timing correction] [-s stereo]\n\n",argv[0]);
      ...
      fprintf(stderr, "timing correction depends on your RPi clock (default %.2f)\n\n",DEFAULTTIMING);
