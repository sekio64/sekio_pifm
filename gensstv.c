...
main (int argc, char *argv[])
{
  ...
  g_rate = RATE;
  g_scottie = 1;
  while ((opt = getopt (argc, argv, "smr:")) != -1)
    {
      switch (opt)
	{
	case 's':
  ...
  if (errorexit)
    {
      fprintf (stderr, "Usage: %s image.png [-s | -m] [-r sample rate]\n",
	       argv[0]);
      fprintf (stderr, " -s for Scottie1 and -m for Martin1\n");
  ...
  if (g_scottie == 0)
    {
      puts ("  encoding = Martin-1");
    }
  else
    {
      puts ("  encoding = Scottie-1");
    }
...
buildaudio ()
{
  ...
  // sekio: we start with a sync pulse in scottie1
  if (g_scottie == 1)
    playtone (1200, 9000);
...
//        printf( "Row [%d] Sample [%d].\n" , y , g_samples ) ;
	...
	{
/*
Scottie 1 - 138.240ms/scan    (.4320ms/pixel @ 320 pixels/line)

(1) Starting sync pulse  (first line only!) 
    9.0ms 1200hz
...
*/
	  ...
	  for (k = 0; k < 320; k++)
	    {
	      playtone (toneval (g[k]), SCOTTIE_PIXLEN);
	    }
	  ...
	  for (k = 0; k < 320; k++)
	    {
	      playtone (toneval (b[k]), SCOTTIE_PIXLEN);
	    }
	  ...
	  for (k = 0; k < 320; k++)
	    {
	      playtone (toneval (r[k]), SCOTTIE_PIXLEN);
	    }


	}			// end martin/scottie decision        
